  ...
  */
  ...
  inline su2double GetPowerStochForcing(unsigned long iPoint, const CConfig *config, const CVariable *node_flow, const CVariable *node_turb, const CGeometry *geometry) {
    ...
    if (IsHybridRANSLES_SST(config->GetKind_HybridRANSLES())) {
      ...
      const su2double delta_i = node_turb->GetDES_FilterWidth(iPoint);
      tkeEstim_i = (lesSensor_i > threshold) ? pow(node_flow->GetStrainMag(iPoint) * delta_i, 2) : 0.0;
    }
    ...
    for (unsigned short iNode = 0; iNode < geometry->nodes->GetnPoint(iPoint); iNode++) {
      ...
      if (IsHybridRANSLES_SST(config->GetKind_HybridRANSLES())) {
        ...
        const su2double delta_j = node_turb->GetDES_FilterWidth(jPoint);
        tkeEstim_j = (lesSensor_j > threshold) ? pow(node_flow->GetStrainMag(jPoint) * delta_j, 2) : 0.0;
      }
