...
void CAvgGrad_Base::SetStressTensor(const su2double *val_primvar,
  ...
  if (sstParsedOptions.uq) {
    ...
    ComputeStressTensor(nDim, tau, val_gradprimvar+1, total_viscosity, Density, 0.0);
  }
...
void CAvgGrad_Base::SetStochSourceMom(const CConfig* config) {
  ...
  if (IsHybridRANSLES_SST(config->GetKind_HybridRANSLES())) {
    ...
    tke_i = (lesMode_i > sensorThreshold) ? nuT_i * StrainMag_i : 0.0;
    tke_j = (lesMode_j > sensorThreshold) ? nuT_j * StrainMag_j : 0.0;
  }
